...
R, `rayshader`, OpenStreetMap... Lots of amazing learning to be had with these geomapping tools. I'm following Tyler Morgan-Wall PhD's [Adding Open Street Map Data to Rayshader Maps in R](https://www.tylermw.com/adding-open-street-map-data-to-rayshader-maps-in-r/) tutorial and learning a lot about the development of beautiful maps with R. Laguna Beach is one of my favorite cities in the USA (_and the world!_) so I applied the concepts from this tutorial to building 2D and 3D maps of Laguna Beach. My work-in-progress and screen captures are below.

Tyler Morgan-Wall's introduction to the `rayshader` package and step-by-step tutorial make the process easy to follow and when you get stuck, you can almost always find an answer on his `rayshader` website or package GitHub (excellent technical documentation and code examples). One last caveat before I dive into my code workflow - Finding high-quality LIDAR or Digital Elevation Models (DEMs) for United States cities was _not easy_. To reproduce the steps below for your location of choice, you may have to get creative about cropping LIDAR/DEMs to zoom or subset the geography that you actually want to model (I also do this below).

# Setup

If you're only wanting to learn, the team at [Shaded Relief](https://shadedrelief.com/) has some high-resolution DEMs for public consumption: [http://shadedrelief.com/SampleElevationModels/](http://shadedrelief.com/SampleElevationModels/)

For the purposes of reproducibility, my starting point was a LIDAR capture (TIF format) of Laguna Canyon in California. This data, made available by the United States Geological Survey's [National Geologic Map Database](https://ngmdb.usgs.gov/topoView/), was captured in 2018 as part of a study on Southern California wildfires. The geo-coordinate data points include latitude, longitude, and elevation, and are available roughly one meter apart (hence the 257 MB filesize). You can download this TIF from my Google Drive here: [https://drive.google.com/Javier/USGS_TIF](https://drive.google.com/file/d/1IfdYa0nYYk7_n1McV5IRhgk-rXJtLA9P/view?usp=sharing) 
...
To better explore the data, lets begin by visualizing the elevation data with `rayshader` defaults to understand the geography that we are going to work with. Before we can render this visual, we need to transform the `laguna` data object (assigned above) to a matrix. For faster rendering and prototyping, you can also resize the matrix using `rayshader::resize_matrix()`. This resizing will downscale your full dataset using bilinear interpolation.
...
I wanted to focus only on downtown Laguna Beach. This only represents a fraction of the raw LIDAR. To get the lat-long range of my desired geography, I used Google Maps to obtain the endpoints of my desired box. Below, we use a custom cropping function (_thank you, Tyler Morgan-Wall_) to accomplish this transformation. 
...
![OSM render using ggplot2](./images/OSM_ggplot2_render.png)
...
![2D render of Laguna Beach](./images/2D_render.png)
...
I've seen some neat examples where developers added 3D polygons to their maps to represent residential housing, commercial properties, and public facilities. I've also read about methods for rendering much higher-resolution maps (this would be great for printing). Creating 3D flyovers with `rayshader` also looks surprisingly enterprise-grade. These are all things I hope to revisit at some point in the near future. Tyler Morgan-Wall knocked it out of the park with the package and his tutorials, so I hope you find his content as useful as I did.

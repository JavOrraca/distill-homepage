...
description: > 
  RStudio's annual conference saw roughly 15,000 attendees for 
  their first global, all-virtual, 24-hour event. I attended
  several sessions throughout the day and I'll highlight my
  favorite data bytes learned that day. I'll also share
  relatable content for better modeling with R.
author:
  - name: Javier Orraca
...
output:
  distill::distill_article:
    ...
    toc: true
    toc_depth: 3
preview: images/rstudio-global-2021.jpg
---
...
Coming from a converted `caret`-ista, it wasn't an easy transition to the full `tidymodels` framework. I liked the way `caret` _felt_, and I did almost everything with it from resampling, modeling, tuning, to model evaluation. Enter `tidymodels`. I've been using this framework for a year and the rate at which this ecosystem has evolved has been impressive.

So what is `tidymodels`? It's a collection of `tidyverse`-friendly packages written with a consistent vocabulary for machine learning, tuning, and developing reproducible modeling workflows. Its core packages include the following:
...
Irregular grid searches help you tune your models by cycling through randomized hyperparameter combinations in an effort to find the best-performing hyperparameters. I've been using the `dials::grid_latin_hypercube()` function to create Latin hypercubes which is explained below, highlighting the benefits of this space-filling design vs a purely randomized irregular grid search. Max Kuhn's _"New in tidymodels"_ rstudio::global event provided me with a fresh reminder about _why_ these methods are beneficial and _how_ they actually optimize performance.

The documentation below was copied almost entirely from Chapters 13 & 14 of Max Kuhn and Julia Silge's [_Tidy Modeling with R_](https://www.tmwr.org/) ("TMWR"). I've deleted certain bits and added minimal helper comments in brackets. If you find yourself statistically programming, bootstrapping / resampling, improving model performance through grid searches, maybe even ensemble stacking, I'd highly recommend that you bookmark the TMWR book.
...
To learn more about advanced _iterative search_ methods such as Bayesian optimization and simulated annealing, please visit [Chapter 14 of TWMR](https://www.tmwr.org/iterative-search.html). Max Kuhn covered these advanced techniques during his `tidymodels` rstudio::global event and I haven't had a chance to try these iterative search methods.

## usemodels

This `tidymodels` package wasn't covered in detail in any of the rstudio::global(2021) events that I attended, but in the spirit of sharing useful tips, tricks, and packages, I'd like to introduce you to Max Kuhn's `usemodels` package (if you aren't already using it and/or familiar with it). This package is not yet on CRAN so you'll need to install it using the following:
...
Similar to `usemodels`, this is a very early-stage package, but two packages that you should start following. There are several AI platforms for producing "model stacks" or "ensemble models" such as [H2O.ai](http://h2o-release.s3.amazonaws.com/h2o/master/1735/docs-website/Ruser/Rinstall.html) and several other open-source libraries. To my knowledge, `stacks` is the first one of these purpose-built for use with `tidymodels`. The following narrative is copied almost directly from the [stacks website](https://stacks.tidymodels.org/). 
